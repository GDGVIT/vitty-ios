			...
			ZStack(alignment: .leading) {
				...
				VStack(alignment: .leading) {
					Text(StringConstants.instructionsHeaders[0])
						...
						.padding(.vertical, 5)
					Text(
						StringConstants.accData[0]
							+ (UserDefaults.standard.string(forKey: "userName") ?? "Anon")
					)
					.foregroundColor(Color.vprimary)
					Text(
						StringConstants.accData[1]
							+ (UserDefaults.standard.string(forKey: "providerId") ?? "")
					)
					.foregroundColor(Color.vprimary)
					Text(
						StringConstants.accData[2]
				...
				VStack(alignment: .leading) {
					...
					VStack(alignment: .leading) {
						...
						ForEach(0..<StringConstants.setupInstructions.count) { point in
							...
							if point == 0 {
								Link(
									destination: URL(string: StringConstants.websiteURL) ?? URL(
										string: "https://dscvit.com/"
									)!,
									label: {
