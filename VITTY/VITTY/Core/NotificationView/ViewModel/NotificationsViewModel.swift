...
class NotificationsViewModel: NSObject, ObservableObject, UNUserNotificationCenterDelegate {
			...
			if let currentDayClasses = timetable[currentDay] {
				if !currentDayClasses.isEmpty {
					for aclass in 0..<currentDayClasses.count {
						let notifsetting = NotificationsSettingsModel(
							id: UUID().uuidString,
							enabled: true,
							day: (day + 1),
							period: aclass,
							location: currentDayClasses[aclass].location ?? "location"
						)
	...
	func updateNotifs(timetable: [String: [Classes]]) {
		...
		if examMode {
			...
			UNUserNotificationCenter.current()
				.getPendingNotificationRequests { allPendingNotifs in
					var idsToRemove: [String] = []
					for notifSetting in self.notifSettings {
						if !notifSetting.enabled
							&& allPendingNotifs.contains(where: { $0.identifier == notifSetting.id }
							)
						{
							idsToRemove.append(notifSetting.id ?? "")
						}
						else if notifSetting.enabled
							&& !allPendingNotifs.contains(where: {
								$0.identifier == notifSetting.id
							})
						{
	...
	func addNotif(timetable: [String: [Classes]], notifInfo: NotificationsSettingsModel) {
		let classs: Classes =
			timetable[StringConstants.notificationDays[notifInfo.day - 1]]?[notifInfo.period]
			?? Classes()

		NotificationsManager.shared.addNotifications(
			id: notifInfo.id ?? "",
			date: classs.startTime ?? Date(),
			day: notifInfo.day,
			courseCode: classs.courseCode ?? "Course Code",
			courseName: classs.courseName ?? "Course Name",
			location: classs.location ?? "Location"
		)
	}
	...
	func setupNotifs(timetable: [String: [Classes]]) {
		...
		for period in self.notifSettings {
			...
			if period.enabled {
				let currClass = timetable[StringConstants.notificationDays[period.day - 1]]?[
					period.period
				]
				...
				NotificationsManager.shared.addNotifications(
					id: period.id ?? "id",
					date: currClass?.startTime ?? Date(),
					day: period.day,
					courseCode: currClass?.courseCode ?? "Course Code",
					courseName: currClass?.courseName ?? "Course Name",
					location: currClass?.location ?? "Location"
				)
			}
	...
	func getNotifPrefs() {
		guard let notifData = UserDefaults.standard.data(forKey: NotificationsViewModel.notifKey)
		else {
			print("No notification settings stored")
			return
		}
		guard
			let notifArray = try? JSONDecoder()
				.decode([NotificationsSettingsModel].self, from: notifData)
		else {
