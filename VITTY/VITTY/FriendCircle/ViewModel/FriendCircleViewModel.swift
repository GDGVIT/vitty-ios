...
class FriendCircleViewModel: ObservableObject {
	...
	func downloadData(
		fromURLRequest urlReq: URLRequest,
		completionHandler: @escaping (_ data: Data?) -> Void
	) {
		URLSession.shared
			.dataTask(with: urlReq) { data, response, error in
				guard let data = data,
					error == nil,
	...
	func searchUsers(token: String, query: String) {
		guard let url = URL(string: "\(APIConstants.base_url)/api/v2/users/search?query=\(query)")
		else {
			return
		}
	...
	func sendFriendRequest(token: String, username: String) {
		guard let url = URL(string: "\(APIConstants.base_url)/api/v2/requests/\(username)/send")
		else {
			return
		}
			...
			if let data = returnedData {
				do {
					...
					DispatchQueue.main.async { [weak self] in
						...
						print(
							"my friend requests ",
							self?.getFriendRequests ?? "no friend requests"
						)
					}
