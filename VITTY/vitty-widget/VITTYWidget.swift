...
struct VITTYProvider: TimelineProvider {
	...
	func getSnapshot(in context: Context, completion: @escaping (Entry) -> Void) {
		...
		guard let date = Calendar.current.date(byAdding: .hour, value: 48, to: Date()) else {
			return
		}
		let VITTYSnap = VITTYWidgetDataModel(
	...
	func getTimeline(in context: Context, completion: @escaping (Timeline<Entry>) -> Void) {
		...
		// fetch the timetable
		TimetableWidgetService()
			.getTimeTableInfo { dataModel in
	...
	func placeholder(in context: Context) -> VITTYEntry {
		VITTYEntry(
			date: Date(),
			dataModel: VITTYWidgetDataModel(
				classInfo: StringConstants.dummyClassArray,
				classesCompleted: 2,
				error: nil
			)
